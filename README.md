...
- *"Refactor the repo"* — `refactor-round`: one measured round on `refactor/round-N` that takes the next steps from the top of that plan — component breakout, then widget adoption, then utility function identification, then design pattern identification and modification, then the sweep to zero — behaviour unchanged, baseline first and last, ending with the new score.
...
| `kit/skills/refactor-round/SKILL.md` | *Refactor the repo*: one measured round that works the plan — breakout, widget adoption, utilities, patterns, sweep |
