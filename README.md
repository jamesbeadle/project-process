...
**2. Measured, ratcheted refactoring — as Claude scripts, not CI.** `kit/refactor/` is the audit and gate that measure a codebase against the rules (file length, function length, else blocks, duplication, comments, magic values, prose, orphan components) and fail when a ratcheted figure is worse than the committed baseline. Installed as `tools/refactor/`. Nothing runs on GitHub: the gate is run by the skills the kit installs in `.claude/skills/` — `refactor-round` (one measured round: baseline first, worst files first, behaviour unchanged, baseline last) and `end-of-day` (the round if one is due, the connector check, the plain-English summary on the day's tasks). Both work on a branch — the round on `refactor/round-N` — commit there, push, and open a pull request for the person to merge (part 6). `playbook.md` is the nine-stage method; the bootstrap takes Stage 0 (the baseline) for a repository that has none. (`--with-ci-gate` installs a GitHub workflow for anyone who does want the gate on their pushes.)
...
Skills (`.claude/skills/<name>/SKILL.md`) are procedures invoked on demand and travel with the clone the same way; the kit installs `refactor-round` and `end-of-day` there. Skills in `~/.claude/skills/` are personal and stay on the machine.

## Onboarding a repository

Run the bootstrap in the repository (it detects the stack — `dotnet-blazor`, `sveltekit` or `generic` — and the default branch, or take `--stack` and `--branch`). Read the block it put at the top of `CLAUDE.md`, and `tools/refactor/rules.json`, which is the repository's own copy of the preset and is never overwritten. Commit what it wrote on a branch (`git switch -c feature/project-process-<version>`), push it and merge the pull request — that merge is where the deploy count starts. From then on `end-of-day` closes each working day and `refactor-round` runs when it is due.
...
| `kit/skills/refactor-round/SKILL.md` | The round: one measured pass of the extraction loop |
